...
impl TranslateService {
    ...
    pub async fn translate_with_params(
        ...
        let request_body =
            TranslateTextRequest::new_with_params(text, target, format, source, model);
        self.post_translate_request(request_body).await
    }

    async fn post_translate_request(
        &mut self,
        request_body: TranslateTextRequest,
    ) -> Result<TranslateTextResponse> {
        let base_url = Url::parse(&format!(
            "{}/v2/{}",
            TRANSLATE_SERVICE_BASE_URL,
            BasicServiceType::Translate.path()
        ))?;
